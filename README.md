...
O botão **Baixar template Excel** fornece os 72 cabeçalhos do arquivo de referência. O índice ambiental compara dois genótipos apenas nas combinações `trial_name | location_name` em que ambos foram avaliados. A média ambiental no eixo X considera todos os genótipos do datacut; o eixo Y mostra a produtividade média de cada genótipo selecionado.
